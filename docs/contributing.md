...
### Dev Builds
While final releases are usually months apart, we do dev builds every commit to main that get published
to **test.pypi.org**:
...
of a recently published version can still fail for a few minutes after a build shows up on (test) PyPI as the package
is replicated to all download mirrors.

To install a devbuild of gitlint:
```sh
